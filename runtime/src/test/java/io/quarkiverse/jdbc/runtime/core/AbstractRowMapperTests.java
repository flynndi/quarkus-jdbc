...
import io.quarkiverse.jdbc.runtime.core.test.ConcretePerson;
import io.quarkiverse.jdbc.runtime.core.test.ConstructorPerson;
import io.quarkiverse.jdbc.runtime.core.test.DatePerson;
import io.quarkiverse.jdbc.runtime.core.test.EmailPerson;
import io.quarkiverse.jdbc.runtime.core.test.Person;
import io.quarkiverse.jdbc.runtime.core.test.SpacePerson;
import io.quarkiverse.jdbc.runtime.support.SQLStateSQLExceptionTranslator;
...
public abstract class AbstractRowMapperTests {
	...
	protected void verifyPersonViaBeanWrapper(Object person) {
		assertThat(getPropertyValue(person, "name")).isEqualTo("Bubba");
		assertThat(getPropertyValue(person, "age")).isEqualTo(22L);
		Date birthDate;
		if (isReadableProperty(person, "birth_date")) {
			birthDate = (Date) getPropertyValue(person, "birth_date");
		}
		else {
			birthDate = (Date) getPropertyValue(person, "birthDate");
		}
		assertThat(birthDate).usingComparator(Date::compareTo).isEqualTo(new java.util.Date(1221222L));
		assertThat(getPropertyValue(person, "balance")).isEqualTo(new BigDecimal("1234.56"));
	}

	private boolean isReadableProperty(Object target, String name) {
		return (findReadMethod(target.getClass(), name) != null || findField(target.getClass(), name) != null);
	}
	...
	private Object getPropertyValue(Object target, String name) {
		...
		throw new IllegalStateException("No readable property '" + name + "'");
	}

	private Method findReadMethod(Class<?> type, String name) {
		String capitalized = Character.toUpperCase(name.charAt(0)) + name.substring(1);
		for (String methodName : new String[] { "get" + capitalized, "is" + capitalized, name }) {
			Class<?> current = type;
			while (current != null) {
				try {
