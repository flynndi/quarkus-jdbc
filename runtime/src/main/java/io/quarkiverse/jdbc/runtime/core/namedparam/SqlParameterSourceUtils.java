...
public abstract class SqlParameterSourceUtils {
	...
	public static SqlParameterSource[] createBatch(Collection<?> candidates) {
		...
		for (Object candidate : candidates) {
			batch[i] = (candidate instanceof SqlParameterSource parameterSource ? parameterSource :
					(candidate instanceof Map map ? new MapSqlParameterSource(map) : new BeanPropertySqlParameterSource(candidate)));
			i++;
		}
		return batch;
	}
