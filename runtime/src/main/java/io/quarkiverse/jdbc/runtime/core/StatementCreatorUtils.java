...
import io.quarkiverse.jdbc.runtime.support.SqlValue;
...
public abstract class StatementCreatorUtils {
	...
	private static void setNull(PreparedStatement ps, int paramIndex, int sqlType, @Nullable String typeName)
		...
		if (sqlType == SqlTypeValue.TYPE_UNKNOWN || (sqlType == Types.OTHER && typeName == null)) {
			...
			Boolean shouldIgnoreGetParameterType = shouldIgnoreGetParameterType();
	...
	static @Nullable Boolean shouldIgnoreGetParameterType() {
		try {
			return ConfigProvider.getConfig().getOptionalValue(IGNORE_GET_PARAMETER_TYPE_PROPERTY_NAME, Boolean.class)
					.orElse(null);
		}
		catch (IllegalStateException ex) {
			logger.debug("No MicroProfile Config is available for property '" +
					IGNORE_GET_PARAMETER_TYPE_PROPERTY_NAME + "'", ex);
			return null;
		}
	}
