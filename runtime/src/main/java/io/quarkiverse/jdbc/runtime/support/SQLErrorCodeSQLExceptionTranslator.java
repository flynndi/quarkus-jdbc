...
import io.quarkiverse.jdbc.runtime.exception.CannotAcquireLockException;
import io.quarkiverse.jdbc.runtime.exception.DataAccessException;
import io.quarkiverse.jdbc.runtime.exception.DataAccessResourceFailureException;
import io.quarkiverse.jdbc.runtime.exception.DataIntegrityViolationException;
import io.quarkiverse.jdbc.runtime.exception.DuplicateKeyException;
import io.quarkiverse.jdbc.runtime.exception.PermissionDeniedDataAccessException;
import io.quarkiverse.jdbc.runtime.exception.TransientDataAccessResourceException;
import io.quarkiverse.jdbc.runtime.exception.BadSqlGrammarException;
...
public class SQLErrorCodeSQLExceptionTranslator extends AbstractFallbackSQLExceptionTranslator {
	...
	public void setDataSource(DataSource dataSource) {
		this.sqlErrorCodes = () -> SQLErrorCodesFactory.getInstance().resolveErrorCodes(dataSource);
	...
	public void setSqlErrorCodes(@Nullable SQLErrorCodes sec) {
		this.sqlErrorCodes = (sec != null ? () -> sec : null);
	}
	...
	public @Nullable SQLErrorCodes getSqlErrorCodes() {
		return (this.sqlErrorCodes != null ? this.sqlErrorCodes.get() : null);
	}
		...
		if (sqlErrorCodes != null) {
			...
			if (errorCode != null) {
				...
				if (Arrays.binarySearch(sqlErrorCodes.getBadSqlGrammarCodes(), errorCode) >= 0) {
					...
					logTranslation(task, sql, sqlEx, false);
					return new io.quarkiverse.jdbc.runtime.exception.DeadlockLoserDataAccessException(buildMessage(task, sql, sqlEx), sqlEx);
				}
				else if (Arrays.binarySearch(sqlErrorCodes.getCannotSerializeTransactionCodes(), errorCode) >= 0) {
					logTranslation(task, sql, sqlEx, false);
					return new io.quarkiverse.jdbc.runtime.exception.CannotSerializeTransactionException(buildMessage(task, sql, sqlEx), sqlEx);
				}
			}
		}
